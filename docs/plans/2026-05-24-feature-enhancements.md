...
3. **RTL handling.** Proper MUI 5 RTL requires three layers working together: (a) the DOM `dir` attribute on the root `<Box>` in `App.tsx`, (b) the MUI theme's `direction` property, and (c) an Emotion `CacheProvider` with `stylis-plugin-rtl` to flip directional CSS properties. Without `stylis-plugin-rtl`, MUI components know they are in RTL mode (via the theme's `direction`) but Emotion-generated CSS like `padding-left`, `margin-right`, `text-align: left`, and `border-left` will not be flipped — this is the standard MUI 5 RTL setup documented at https://mui.com/material-ui/customization/right-to-left/. The `stylis-plugin-rtl` and `stylis` packages must be installed (`npm install stylis-plugin-rtl stylis`). A `src/rtlCache.ts` module creates the RTL Emotion cache: `createCache({ key: 'muirtl', stylisPlugins: [prefixer, rtlPlugin] })`. In `App.tsx`, `ThemedApp` reads the current locale and conditionally wraps `ThemeProvider` in a `CacheProvider` using the RTL cache (when `locale === 'he'`) or a default LTR cache (when `locale === 'en'`). The `ThemeProvider` is placed **inside** `CacheProvider`, which is inside `LanguageProvider`, so both the cache and theme react to locale changes. A `useDirectionalTheme()` hook in `src/theme.ts` calls `useMemo(() => createTheme({ ...baseThemeOptions, direction: locale === 'he' ? 'rtl' : 'ltr' }), [locale])` to produce a locale-aware theme on each locale change. The root `<Box>` also sets `dir="rtl"` or `dir="ltr"` to match. The existing inline `dir="rtl"` blocks in `StartScreen` and `GameOverScreen` are removed since the whole app handles directionality globally.
...
### Task 4: Install stylis-plugin-rtl, wire LanguageProvider + CacheProvider + ThemeProvider into App, add dynamic RTL theme, and add AppBar with hamburger language toggle

**Files:**
- Create: `src/rtlCache.ts`
- Modify: `src/App.tsx`
- Modify: `src/theme.ts`
- Create: `src/components/LanguageToggleBar.tsx`

- [ ] **Step 1: Install stylis-plugin-rtl**

MUI 5 RTL requires `stylis-plugin-rtl` to flip directional CSS properties (padding, margins, text-align, borders, etc.) in Emotion-generated styles. Without it, setting `direction: 'rtl'` on the theme tells MUI components they are in RTL mode, but the actual CSS output remains LTR. `stylis` is already a transitive dependency of `@emotion/cache` but is listed explicitly for clarity.

Run: `cd /Users/develeap/private_gits/car_models/.worktrees/feature-enhancements && npm install stylis-plugin-rtl stylis`
Expected: Packages installed successfully, `package.json` updated.

- [ ] **Step 2: Write failing test — skip for this task (UI wiring)**

This task is UI wiring that will be verified via the full E2E suite in Task 8. No isolated unit test is practical for the AppBar + CacheProvider + LanguageProvider integration. Proceed directly to implementation.

- [ ] **Step 3: Create RTL Emotion cache module**
 ...
 * This ensures directional CSS properties (padding-left, margin-right,
 * text-align, border-left, etc.) are automatically flipped by stylis.
 */
...
- [ ] **Step 4: Refactor theme.ts to support dynamic direction**
...
- [ ] **Step 5: Implement LanguageToggleBar component**
...
- [ ] **Step 6: Update App.tsx — LanguageProvider outermost, CacheProvider + ThemeProvider inside with dynamic RTL cache and theme**

The critical architecture here: `LanguageProvider` wraps `CacheProvider` which wraps `ThemeProvider`, so both the Emotion cache and theme react to locale changes. The `CacheProvider` from `@emotion/react` receives either the RTL cache (with `stylis-plugin-rtl`) or the LTR cache depending on the active locale. This is what actually flips directional CSS properties in MUI's Emotion-generated styles. The `useDirectionalTheme` hook creates a theme with `direction: 'rtl'` or `'ltr'` matching the active locale, which tells MUI components their layout direction. The DOM `dir` attribute is also set on the root `Box` for native browser RTL behavior.
...
```bash
cd /Users/develeap/private_gits/car_models/.worktrees/feature-enhancements
git add src/rtlCache.ts src/App.tsx src/theme.ts src/components/LanguageToggleBar.tsx package.json package-lock.json
git commit -m "feat: add LanguageProvider, CacheProvider with stylis-plugin-rtl, dynamic RTL theme, and hamburger language toggle"
```
